...
test("protocol errors use the JSON-RPC and MCP codes", async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      [-32600, null], // []
      [-32600, null], // "string"
      [-32600, 1], // missing jsonrpc, but the id is usable
      [-32600, null], // id: null
      [-32600, null], // id: 1.5
      [-32600, null], // id: true
    ],
...
test("research: default role, flags, environment, usage, and artifacts", async (t) => {
  ...
  assert.match(invocation.argv.at(-1), /^Task:\nInspect the fixture and summarize it\./);
  assert.match(invocation.argv.at(-1), /Constraints supplied by the caller:\n- Do not edit files\./);
...
// Polls until the single run's result.json exists, then returns it.
...
test("a cancelled request terminates Pi, gets no response, and is recorded", async (t) => {
  ...
  // The server writes result.json before it would respond, and stdout is
  // ordered, so once the ping is answered any response for `id` would be here.
  assert.equal((await client.request("ping")).resultType, "complete");
